    ...
    public class ExtruderNozzlesViewModel : DatabaseEntityViewModel
    {
        ...
        public override ObservableCollection<Error> GetCustomErrors()
        {
            ...
            for (int i = 0; i < ExtruderNozzles.Count; i++)
            {
                ...
                if (Math.Sign(ExtruderNozzles[i].Consumption) < 0)
                {
                    errors.Add(new Error
                    {
                        Content = string.Format(LocalizationManager.Instance.GetValue("Window.ExtruderNozzles.Errors.ConsumptionIsNegative"), entryIndex)
                    });
                }
            }
            return errors;
